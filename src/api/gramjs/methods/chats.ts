...
export async function fetchChats({
  ...
  const requestedFolderId = folderId ?? (archived ? ARCHIVED_FOLDER_ID : undefined);
  ...
  const resultPinned = withPinned
    ? await invokeRequest(new GramJs.messages.GetPinnedDialogs({
      folderId: requestedFolderId ?? ALL_FOLDER_ID,
    }))
    : undefined;
  ...
  dialogs.forEach((dialog) => {
    if (
      ...
      || (requestedFolderId === undefined && dialog.folderId === ARCHIVED_FOLDER_ID)
      || (requestedFolderId !== undefined && dialog.folderId !== requestedFolderId)
    ) {
      return;
    }
