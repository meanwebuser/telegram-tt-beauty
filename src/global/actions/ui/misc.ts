...
import { getAiSidebarOpenPlan } from '../../helpers/aiWorkspaceSync';
...
addActionHandler('openAiSidebar', (global, actions, payload): ActionReturnType => {
  const { chatId, tabId = getCurrentTabId() } = payload;
  const openPlan = getAiSidebarOpenPlan(global, chatId);
  if (openPlan.shouldCreateTopic) {
    // Resolve or create the workspace topic for this chat in the background.
    // Keep the existing topic id if we already have one, otherwise pass 0 and
    // let the createAiTopic handler fill in the real id once it's allocated.
    void actions.createAiTopic({
      sourceChat: { id: chatId, type: 'user' },
      tabId,
    });
  }
  ...
  return updateTabState(global, {
    aiSidebar: {
      chatId,
      topicId: openPlan.topicId,
    },
  }, tabId);
});

addActionHandler('closeAiSidebar', (global, actions, payload): ActionReturnType => {
  const { tabId = getCurrentTabId() } = payload || {};
  return updateTabState(global, {
    aiSidebar: undefined,
  }, tabId);
