...
addActionHandler('setAiAssistantSettings', (global, actions, payload): ActionReturnType => {
  const { apiKey, endpoint, model, stream, temperature, maxTokens } = payload || {};
  const aiAssistant = { ...(global.settings.byKey.aiAssistant || {}) };
  if (apiKey !== undefined) aiAssistant.apiKey = apiKey;
  if (endpoint !== undefined) aiAssistant.endpoint = endpoint;
  if (model !== undefined) aiAssistant.model = model;
  if (stream !== undefined) aiAssistant.stream = stream;
  if (temperature !== undefined) aiAssistant.temperature = temperature;
  if (maxTokens !== undefined) aiAssistant.maxTokens = maxTokens;
  // The aiAssistant is enabled the moment a key is present so the button
  // appears and the sidebar flow stays one click away.
  aiAssistant.isEnabled = Boolean(aiAssistant.apiKey);
  global = replaceSettings(global, { aiAssistant });
  setGlobal(global);
});
