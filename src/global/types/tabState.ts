...
export type TabState = {
  ...
  // Tracks the right-sidebar AI workspace opened for the current source chat.
  aiSidebar?: {
    chatId: string;
    topicId: number;
    /** Increments with every new prompt; sidebar watches it to know when to
     * start a fresh streaming buffer. */
    streamNonce?: number;
    /** Live model output. Append-only while streaming, frozen on completion. */
    streamText?: string;
    /** Set to a non-empty string when the latest request failed. Cleared on
     * a new prompt. */
    streamError?: string;
