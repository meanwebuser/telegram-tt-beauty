...
const RoundVideo: FC<OwnProps> = ({
  ...
  const handleTranscribe = useLastCallback(() => {
    if (window.__tgWhisperTranscribeMedia?.(mediaData, message)) return;
    transcribeAudio({ chatId: message.chatId, messageId: message.id });
  });

  useEffect(() => {
    if (canTranscribe && mediaData && !isTranscribing && !isTranscribed && !isTranscriptionError) {
      window.__tgWhisperQueueMedia?.(mediaData, message);
    }
  }, [canTranscribe, isTranscribing, isTranscribed, isTranscriptionError, mediaData, message]);
