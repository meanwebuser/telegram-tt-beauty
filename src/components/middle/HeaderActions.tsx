...
import type { AiAssistantSettings, IAnchorPosition, MessageListType, ThreadId, TranslationTone } from '../../types';
...
const HeaderActions: FC<OwnProps & StateProps> = ({
  ...
  const handleAiSidebarClick = useLastCallback(() => {
    if (!chatId) return;
    openAiSidebar({ chatId });
  });
...
export default memo(withGlobal<OwnProps>(
  (global, {
    ...
    if (!chat || isRestricted || selectIsInSelectMode(global)) {
      ...
      } as unknown as Complete<StateProps>;
    }
    ...
    // no extra selector work needed for the sidebar — it loads its own chat context
