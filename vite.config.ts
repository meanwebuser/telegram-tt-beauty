...
export default defineConfig(({ mode }): UserConfig => {
  ...
  // Default brand: "TChat" — a non-Telegram name, so the built page does not
  // impersonate web.telegram.org and is not flagged as phishing by Google Safe
  // Browsing. Set APP_TITLE / APP_DESCRIPTION at build time to override.
  ...
  // og:image and twitter:image use a separate icon (TChat branding) so the
  // page does not advertise the Telegram paperplane when shared or scanned
  // by Google Safe Browsing. Set TG_OG_IMAGE / TG_FAVICON at build time to
  // override. The default favicon is public/favicon.svg (overwritten to the
  // TChat mark — see public/favicon.svg).
  const ogImage = env.OG_IMAGE || 'tchat-icon-192';
  ...
  setViteEnv({
    ...
    // Force the local telegram-tt-proxy at /proxy/ for Telegram connections.
    // Set TG_USE_TELEGRAM_PROXY=0 to force direct (e.g. local dev where the
    // proxy isn't available). When unset, the client auto-detects from
    // window.location.host (tg/tgb.example.com use the proxy).
    TG_USE_TELEGRAM_PROXY: env.TG_USE_TELEGRAM_PROXY || '',
  });
